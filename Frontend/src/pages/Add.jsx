...
function Add() {
  ...
  const {allTypes, setProducts, products, getProducts} = useContext(AppContext)
  ...
  const onSubmitHandler = async(e) => {
    ...
    try {
      const result = await api.post('/products/create', formdata,
        {
  ...
  return (
    <div className='w-full md:w-[85vw] xl:w-[60vw] min-h-screen bg-gray-100 m-auto p-5 mt-5'>
        <ToastContainer
            ...
            transition={Bounce}
            />
        <h1 className='text-[1rem] md:text-[1.2rem] text-gray-700 font-semibold'>Add Product</h1>
        <form>
            ...
            <div className='h-20 flex gap-2 items-center'>
              ...
              <input hidden onChange={imageOnChangeHandler} id='img-input' type="file" multiple/>
            </div>
            ...
            <div className='h-20 flex gap-10 items-center'>
              <div className='flex gap-2 relative'>
                ...
                <input onFocus={() => setTypeBox(true)} onBlur={() => setTypeBox(false)} onChange={tagChangeHandler} autoComplete='off' className='input w-fit md:w-80 h-10 p-2 border-1 text-wrap border-s-gray-700 outline-none' id='tag' value={tag} type='text' placeholder='Add tags'/>
                <div className={`${typeBox? '': 'hidden'} input.active:hidden scrollable-container absolute z-10 left-26 top-12 lg:top-12 list-none bg-white text-gray-700 text-[1rem] w-40 max-h-40 overflow-auto flex flex-col gap-1 border-1 border-gray-300`}>
                  {filtered.map((tag) => {
            ...
            <div className='flex gap-5 mt-8'>
              {!loading?<button onClick={onSubmitHandler} className='h-10 w-fit px-4 py-1 bg-black text-white cursor-pointer'>Add Product</button>
